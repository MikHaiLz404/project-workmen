...
impl AppState {
  ...
  pub fn launched(app_delegate: &Object) {
    // Workmen patch: wrap the body in `catch_unwind` because macOS
    // Sequoia 26.x occasionally panics inside apply_activation_policy
    // or MainThreadMarker::new().unwrap() during the
    // applicationDidFinishLaunching notification. Because
    // did_finish_launching is an `extern "C"` callback, Rust cannot
    // unwind a panic from there -- SIGABRT fires before the window
    // renders. Catching converts the panic into a stderr log; the app
    // may lose focus acquisition but continues running. See
    // `patches/tao-0.35.3/README.md` for context.
    ...
    if let Err(payload) = launch_result {
      eprintln!(
        "[workmen/tao-patch] swallowed panic from AppState::launched on macOS: {:?}",
        payload
      );
    }
    HANDLER.set_ready();
